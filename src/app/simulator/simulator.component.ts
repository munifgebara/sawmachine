...
export class SimulatorComponent implements OnInit {
  ...
  initState() {
    ...
    layout.parts.forEach(p => {
      this.mapLayoutParts[`P${p.part.id}`] = { w: p.part.c, h: p.part.l, r: p.part.rotate, x: p.origin.x, y: p.origin.y };
    });
  ...
  sawCut() {
    ...
    if (!(part.y + 0.1 >= this.sawY - this.sawThickness && (part.y - part.h) <= this.sawY + this.sawThickness + 0.1)) {
      this.addMessage(`A parte ${part.info} não está na serra  ${JSON.stringify(part)}`);
    }
  ...
  info2(part) {
    ...
    if (problemas) {
      return ` Original ${p.w}x${p.h}x${p.r ? 'R' : ''} dW${dw} dH${dh}  ${this.r100(p.x)}x${this.r100(p.y)} `;
    }
    else {
      return `OK`;
    }
  ...
  info3(part) { // Para calcular o erro
    let p = this.mapLayoutParts[part.id];
  ...
  cutWorkPart(part: Part, cutToPart: boolean) {
    ...
    if (cutToPart) {
      ...
      this.instructionStack.push({ part: part, attribute: 'y', targetValue: 3100 + playout.y, cut: undefined, speed: this.defaultSpeed, done: false });
      this.instructionStack.push({ part: part, attribute: 'x', targetValue: 100 + playout.x, cut: undefined, speed: this.defaultSpeed, done: false });
    }
    else {
      this.adjustPhase(part);
      this.instructionStack.push({ part: part, attribute: 'y', targetValue: 0, cut: undefined, speed: this.defaultSpeed, done: false });
      if (newPart) this.instructionStack.push({ part: newPart, attribute: 'x', targetValue: newPart.w + this.defaultSpace, cut: undefined, speed: this.defaultSpeed, done: false });
    }
  ...
  adjustPhase(part: Part): any {
    ...
    if (deltaPhase > 0) {
      this.addMessage(`Mudando de fase da ${currentCut.phase} ===> ${nextCut.phase}`, true);
      this.turnPart90(part);
      this.pushNextCutInstruction(part);
    }
    else if (deltaPhase < 0) {
      this.addMessage(`Corte ${this.iCut} Mudando de fase da ${currentCut.phase} ===> ${nextCut.phase}`, true);
      part.info += "(Sobra)";
      part.sobra = true;
      this.setUrgenturgentInstruction({ part: part, attribute: 'y', targetValue: 10000, cut: undefined, speed: 5 * this.defaultSpeed, done: false });

      let oldWorkPart = this.workPartsStack.pop();
      while (oldWorkPart && oldWorkPart.sobra) {    // Muito perigo
  ...
  tansform(part: Part) {
    return `rotate(${part.rotation} ${part.x} ${part.y}) translate (${part.x} ${part.y}) scale(${part.scale} ${-part.scale}) translate (${-part.x} ${-part.y})`;
  }
  tansform2(part: Part) {
    return `rotate(${part.rotation} ${part.x} ${part.y}) translate (${part.x} ${part.y}) scale(${1} ${-1}) translate (${-part.x} ${-part.y})`;
  }
  ...
  auto() {
    if (!this.automatic) {
      this.automatic = true;
      this.interval = setInterval(() => { this.tick(this.quick); }, 5);
    }
  }
  ...
  setUrgenturgentInstruction(ui: Instruction) {
    ...
    if (this.urgentInstruction && !this.urgentInstruction.done) {
      this.urgentInstruction.part[this.urgentInstruction.attribute] = this.urgentInstruction.targetValue;
  ...
  tick(manual = false) {
    ...
    if (this.urgentInstruction && !this.urgentInstruction.done && !manual) {
      ...
      this.urgentInstruction.done = (this.calculateDistance(this.urgentInstruction) <= Math.abs(this.urgentInstruction.speed))
      ...
      if (this.instructionStack.length > 0) {
        ...
        this.stop();
        this.addMessage("FIM  ");
        this.workPartsStack.forEach(wp => console.log(JSON.stringify(wp.sobra)));


        this.partsReadyList.sort((a, b) => a > b ? 1 : -1);
        this.partsReadyList.forEach(part => {
          console.log(` ${part.info} cords:${part.x}x${part.y} calc:${this.r100(part.w)}x${this.r100(part.h)} ${this.info2(part)}  `);
        });
        console.log(`Erro H ${this.partsReadyList.reduce((s, p) => s + this.info3(p).h, 0)}`);
        console.log(`Erro W ${this.partsReadyList.reduce((s, p) => s + this.info3(p).w, 0)}`);
        console.log(`Erro Total ${this.partsReadyList.reduce((s, p) => s + this.info3(p).w + this.info3(p).h, 0)}`);
      ...
      if (this.calculateDistance(this.currentInstruction) >= Math.abs(this.currentInstruction.speed) && !manual) {
        if (manual) {
        ...
        this.currentInstruction.part[this.currentInstruction.attribute] = this.currentInstruction.targetValue;
        this.distance = 0;
  ...
  private calculateDistance(instruction: Instruction) {
    return Math.abs(instruction.part[instruction.attribute] - instruction.targetValue);
  }
  ...
  private normalEvolute(instruction: Instruction) {
    ...
    else if (instruction.part[instruction.attribute] > instruction.targetValue) {
      instruction.part[instruction.attribute] -= instruction.speed;
    }
  }
  ...
  turnPart90(part) {
    ...
    //part.rotation -= 90;
    //this.setUrgenturgentInstruction({ part: part, attribute: "rotation", cut: undefined, done: false, speed: 2, targetValue: part.rotation + 90 });
  }
  ...
  corrigeNaMarra(part) {
    ...
    if (Math.abs(this.r100(playout.w - part.w)) > 0) {
      this.addMessage(`corrigindo W de ${part.info}  ${part.w} ${playout.w}  `, true);
      part.w = playout.w;
    }
    if (Math.abs(this.r100(playout.h - part.h)) > 0) {
      this.addMessage(`corrigindo H de ${part.info}  ${part.h} ${playout.h}  `, true);
      part.h = playout.h;
    }
  }
